...
require_once(\app\tools::build_path('lib/users.inc.php'));
require_once(\app\tools::build_path('lib/mysql.inc.php'));
require_once(\app\tools::build_path('lib/crypt.inc.php'));
...
class login extends \controller\unauthenticated_controller {
	...
	public function 			show_login($_error_code, $_username) {
		...
		if(null!==$this->get_di()->get_user() || (null==$_error_code && $request->has_cookie('sessid'))) {
			return $this->reply_with_redirection(\app\tools::build_url('index.php'),
	...
	public function				perform_pass_recovery($_email) {
		...
		$email=trim($_email);
		if(!substr_count($email,'@screenrol.es') && !substr_count($email,'@screenrol.com')){
			return $this->reply_with_redirection(
				\app\tools::build_url('recuperar_pass.php?msg=1'), \app\response::status_code_303_see_other, []);
		}
		...
		try {
			...
			return $this->reply_with_redirection(
				\app\tools::build_url('recuperar_pass.php?sent=1'), \app\response::status_code_303_see_other, []);
		}
		catch(\Exception $e) {
			log_dash('peticion_reseteo_pass',$email, "ERR\n HASH: $hash");
			return $this->reply_with_redirection(
				\app\tools::build_url('recuperar_pass.php?msg=2'), \app\response::status_code_303_see_other, []);
		}
	}
	...
	public function				show_reset_pass($_email, $_hash, $_msg, $_sent) {
		...
		if($new_hash!=$_hash) {
			return $this->reply_with_redirection(
				\app\tools::build_url('recuperar_pass.php?msg=3'), \app\response::status_code_303_see_other, []);
		}
	...
	public function				perform_pass_change($_email, $_hash, $_pass, $_check_pass) {
		...
		try {
			...
			$user=$em->get_one_by_field(\model\app\usuario::class,'email',trim($_email));
			if(null===$user) {
			...
			return $this->reply_with_redirection(
				\app\tools::build_url('reset_pass.php?email='.$_email.'&seed='.$_hash.'&sent='.$user->get_usuario()), \app\response::status_code_303_see_other, []);
		}
		catch(\Exception $e) {

			$code=$e->getCode();
			//Lol...
			$url=$code < 3 ? 'reset_pass.php?email='.$_email.'&seed='.$_hash.'&msg='.$code : 'recuperar_pass.php?msg='.$code;
			return $this->reply_with_redirection(
	...
	public function				do_login($_user, $_pass, $_remember) {
		...
		// $user=user_perform_login($_user, $_pass, $this->get_di());

		$username=mysql_real_escape_string(trim(strtolower($_user)));
		$pass=md5(mysql_real_escape_string($_pass));

		$em=$this->get_di()->get_em();
		$user=$em->get_one_by_criteria(
			\model\app\usuario::class,
			new \orm\andgroup(
				new \orm\param('usuario',\orm\param::equal,$username),
				new \orm\param('password',\orm\param::equal,$pass)
			)
		);

		if(null===$user) {
			return $this->reply_with_redirection(\app\tools::build_url('login.php?err=in&username='.$_user),
				\app\response::status_code_303_see_other, []);
		}
		...
		if($PHPSSID){
			...
			$sess->set_phpsessid($PHPSSID)
				...
				->set_persist($_remember ? true : false);
			$em->create($sess);
		}
	...
	public function				do_logout() {
		...
		return $this->reply_with_redirection('login.php', \app\response::status_code_303_see_other, []);
	}

	private static function		generate_hash($_email) {

		return md5($_email.'C0mv1v13nd0');
	}
}
